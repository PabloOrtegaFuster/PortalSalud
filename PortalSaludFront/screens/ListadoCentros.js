...
export default function ListadoCentros({ navigation, route }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.post(SERVER + 'centros/listado', null, {
        headers: {
  ...
  const deleteCentro = async (id) => {
    try {
      ...
      const response = await axios.post(SERVER + 'centros/borrar', Data, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'x-refresh-token': refreshToken,
        }
      ...
      if (response.data.success) {
        ...
        if (Platform.OS === 'web') {
          window.alert(
            "El centro ha sido eliminado con éxito."
          );
        } else {
        ...
        if (Platform.OS === 'web') {
          window.alert(
            "Ocurrió un error al intentar borrar el centro. Inténtalo de nuevo."
          );
        } else {
  ...
  const handlePress = (item) => {
    if (accion == "modificar") {
      navigation.navigate("EditarCentro", { centro: item.id })
    }
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <SafeAreaView style={styles(height, width).header}>
        ...
        <Text style={{ flexWrap: 'wrap', flex: 15, fontFamily: 'Nunito', textAlign: 'center', fontSize: titleFont }}>Centros</Text>
